...
export const RouterView = React.forwardRef<any, RouterViewProps>((props, reactRef) => {
  const {
    routeToDisplay,
    matchedRouteRef,
    viewRef,
    ...
    const name = computed(() => ('name' in props) ? props.name : 'default')
    ...
    watch(
      () => [viewRef.value, matchedRouteRef.value, name.value] as const,
  ...
  const ViewComponent = matchedRoute && matchedRoute.components[name.value!]
  ...
  const routePropsOption = matchedRoute!.props[name.value!]
  ...
  return React.createElement(
    ...
    assign({}, routeProps, props, {
      // onVnodeUnmounted,
      ref: (instance: any) => {
        viewRef.value = instance
        if (typeof reactRef === 'function') {
          reactRef(instance)
        } else {
          if (reactRef) {
