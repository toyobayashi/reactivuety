...
export function queuePostFlushCb (cb: SchedulerCbs): void {
...
const getId = (job: SchedulerJob | SchedulerCb): number =>
...
function checkRecursiveUpdates (seen: CountMap, fn: SchedulerJob | SchedulerCb): void {
  if (!seen.has(fn)) {
    seen.set(fn, 1)
  } else {
